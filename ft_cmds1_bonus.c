...
/*   Updated: 2024/01/28 16:37:24 by ttrave           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	***ft_get_cmds(int argc, char **argv)
{
	...
	here_doc = ft_strcmp(HD, argv[1]);
	cmds = malloc((argc - here_doc - 2) * sizeof(char **));
	if (!cmds)
		return (NULL);
	cmds[argc - here_doc - 3] = NULL;
	i_argv = 2 + here_doc;
	i_cmd = 0;
	while (i_cmd < (size_t)argc - 3 - (size_t)here_doc)
	{
